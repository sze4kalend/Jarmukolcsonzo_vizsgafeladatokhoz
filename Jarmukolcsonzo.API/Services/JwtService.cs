...
namespace Jarmukolcsonzo.API.Services
{
    public class JwtManagerService
    {
        ...
        {
            byte[] key = Encoding.UTF8.GetBytes(_configuration["JWT:Key"] ?? throw new NullReferenceException("JWT:Key is not configured."));
            SigningCredentials signIn = new(new SymmetricSecurityKey(key), SecurityAlgorithms.HmacSha256);
            ...
            return new JwtToken(accessToken, GenerateRefreshToken());
        }
